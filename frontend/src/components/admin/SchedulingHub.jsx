    ...
    return (
      <>
        ...
        <div style={{
          ...
          <div style={{ padding: '1rem 1.25rem', borderBottom: '1px solid #E5E7EB', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#F8FAFC' }}>
          ...
          <div style={{ padding: '1rem 1.25rem', flex: 1, maxWidth: '1200px', margin: '0 auto', width: '100%', display: 'flex', gap: '2rem' }}>
            ...
            {selectedCaregiverId && (
              <form onSubmit={handleCreateSubmit}>
                ...
                <button type='submit' className='btn btn-primary' disabled={saving || ((formData.scheduleType === 'multi-day' || formData.scheduleType === 'bi-weekly') && selectedDays.length === 0)} style={{ width: '100%' }}>
          ...
          {selectedCaregiverId && caregiverSchedules.length > 0 && (
              <div style={{ position: 'sticky', top: '1rem' }}>
                ...
                {Object.entries(groupSchedules().recurringByDay).map(([dayNum, scheds]) => (
                  <div key={dayNum} style={{ marginBottom: '0.75rem' }}>
                    <div style={{ fontSize: '0.78rem', fontWeight: '700', color: '#1E40AF', marginBottom: '0.3rem' }}>{getDayName(parseInt(dayNum))}</div>
                    {scheds.map(s => (
                      <div key={s.id} onClick={() => openEditModal(s)}
                        style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.4rem 0.6rem', background: '#F3F4F6', borderRadius: '6px', marginBottom: '0.25rem', fontSize: '0.82rem', cursor: 'pointer' }}>
                        <div>
                          <span style={{ fontWeight: '600' }}>{getClientName(s.client_id)}</span>
                          <span style={{ color: '#6B7280', marginLeft: '0.5rem' }}>{formatTime(s.start_time)}–{formatTime(s.end_time)}</span>
                          {s.frequency === 'biweekly' && <span style={{ marginLeft: '0.4rem', padding: '0.1rem 0.3rem', background: '#FFEDD5', color: '#C2410C', borderRadius: '4px', fontSize: '0.68rem' }}>Bi-Wk</span>}
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); handleDeleteSchedule(s.id); }} style={{ background: 'none', border: 'none', color: '#DC2626', cursor: 'pointer', fontSize: '0.9rem' }}>🗑</button>
                      </div>
                    ))}
                  </div>
                ))}
                {Object.entries(groupSchedules().oneTimeByDate).sort().map(([dateKey, scheds]) => (
                  <div key={dateKey} style={{ marginBottom: '0.75rem' }}>
                    <div style={{ fontSize: '0.78rem', fontWeight: '700', color: '#1E40AF', marginBottom: '0.3rem' }}>{new Date(dateKey + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}</div>
                    {scheds.map(s => (
                      <div key={s.id} onClick={() => openEditModal(s)}
                        style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.4rem 0.6rem', background: '#F3F4F6', borderRadius: '6px', marginBottom: '0.25rem', fontSize: '0.82rem', cursor: 'pointer' }}>
                        <div><span style={{ fontWeight: '600' }}>{getClientName(s.client_id)}</span><span style={{ color: '#6B7280', marginLeft: '0.5rem' }}>{formatTime(s.start_time)}–{formatTime(s.end_time)}</span></div>
                        <button onClick={(e) => { e.stopPropagation(); handleDeleteSchedule(s.id); }} style={{ background: 'none', border: 'none', color: '#DC2626', cursor: 'pointer', fontSize: '0.9rem' }}>🗑</button>
                      </div>
                    ))}
                  </div>
